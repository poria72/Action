...
L_Original = zeros(size(featureVector,2),1);
for i=1:size(featureVector,2)
    L_Original(i,1) = (LabelTranslate(featureVector(i).Label,Setting.DatasetName));
end
...
C = zeros(classNo); % Confusion Matrix
...
for i=1:Train_TestNumber
    ...
    p(:,1)=VN_test; %video Number
    p(:,2)=L_test; % Label Test
    p(:,3)=predict_label;
    ind_=find(p(:,1)==p(1,1)); %find all Batches of a Video
    vot = majorityvote(p(ind_,3)); %majority vote along all batches
    if(size(vot,2)==0)
        vot=0;
    end
    lbl = p(1,2);
